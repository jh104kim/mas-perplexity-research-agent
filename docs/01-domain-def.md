...
| D-02 | Mock 우선 MVP 전략 | `02-prd.md`, `04-tdd-plan.md` |
...
그래서 MVP에서는 먼저 `Mock Executor`로 전체 흐름을 검증한다. 흐름이 안정되면 실제 Perplexity 호출부만 교체한다.
...
| Perplexity Executor | Perplexity MCP/API를 호출하는 실제 Executor |
...
- `report.html` 생성
- `run_log.json` 생성
- Perplexity Executor를 교체 가능한 구조로 추가
...
| 실제 Perplexity API가 처음부터 필요한가? | 아니다. Mock으로 먼저 검증한다. |
...
- DS-08: API Key가 로그나 보고서에 노출되지 않는다.
