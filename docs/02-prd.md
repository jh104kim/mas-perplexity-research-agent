...
처음부터 Perplexity API를 연결하면 비용과 디버깅 부담이 커진다. 따라서 MVP는 `mock` 모드로 전체 흐름을 먼저 검증하고, 이후 `perplexity` 모드로 Executor만 교체한다.
...
4. `approve`이면 리서치가 실행된다.
5. 결과가 파싱되고 평가된다.
6. 85점 미만이면 최대 3회까지 개선 루프를 돈다.
7. 최종 Markdown 보고서와 HTML 보고서를 저장한다.
...
| FR-02 | 실행 모드는 `mock`, `perplexity`를 지원한다. | Must |
...
| FR-16 | Perplexity Executor는 나중에 교체 가능한 구조로 붙인다. | Should |
...
| NFR-02 | API Key는 로그와 보고서에 노출하지 않는다. |
| NFR-03 | 실패 시 어느 노드에서 실패했는지 기록한다. |
| NFR-04 | 구조는 작고 읽기 쉬워야 한다. |
| NFR-05 | 실제 API 연결 전 mock mode 테스트가 먼저 통과해야 한다. |
...
| FR-02 | D-02, DS-01 | Executor 선택 정책 | `test_executor_mock.py` |
...
| FR-16 | D-02 | executor interface | `test_graph_flow.py` |
| NFR-01 | 전체 | tests 폴더 | 전체 pytest |
| NFR-02 | DS-08 | masking/log policy | `test_graph_flow.py`, `test_report_builder.py` |
| NFR-03 | DS-01 | `run_log.json` | failure logging scenario |
| NFR-04 | 전체 | 작은 모듈 구조 | 코드 리뷰 |
| NFR-05 | D-02 | phase gate | roadmap checklist |

## CLI 예시

```bash
python -m app.cli --topic "LangGraph MAS Workflow" --mode mock
python -m app.cli --topic "LangGraph MAS Workflow" --mode perplexity
```
...
- AC-07: `perplexity` 모드 추가 시 Graph 구조를 바꾸지 않는다.
- AC-08: API Key가 로그와 보고서에 노출되지 않는다.
